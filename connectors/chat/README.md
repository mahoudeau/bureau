...
4. **Make it proactive**: the server teaches connected sessions the shift discipline through its own MCP instructions, but instructions answer *how*, not *whether*. To make every conversation reach for the Bureau on its own, add this to the app's global preferences (the envoy's default name is consul; rename if you like):
