...
2. **Read your inbox.** `POST {{BUREAU_URL}}/api/messages/inbox` with `{"for":"{{WORKER_NAME}}"}`. Act on what is addressed to you; broadcasts are context.
3. **Claim.** `POST {{BUREAU_URL}}/api/tasks/claim` with `{"agent":"{{WORKER_NAME}}"}`. You get the highest-priority queued mission and a lease. If the response is `queue_empty`, heartbeat `idle` and end the shift cleanly.
