...
- **Provenance is non-negotiable, and it is a floor, not just a proposing-time discipline.** A promoted fact carries `(source: [[...]])` wikilinks to the journal lines, debriefs, or missions that earned it. If you cannot source a claim, you do not propose it — and if an unsourced item somehow reaches you approved anyway, you refuse to apply it (step 2 below): the floor holds at both ends, not just the honor system at proposal time.
...
6. **Debrief and close.** Append the trajectory-shaped debrief (question / evidence / alternatives / decision / outcome / revision — docs/brain-format.md's debrief grammar) to `projects/bureau/STATE.md`, then set status `review` with a note counting the items. Your missions end at `review` while the gate holds, never `done`.
