...
(function () {
  ...
  function injectStyle() {
    ...
    style.textContent = [
      ...
      // t-136 (rebased t-206): font-weight/font-size/line-height/color
      // dropped from .v2-task-card__title below — molecules.css's doubled-
      // class `.v2-task-card__title.v2-task-card__title` (t-77, specificity
      // 0,2,0,0) declares all four and wins over this single-class injected
      // rule (0,1,0,0) regardless of source order; re-confirmed directly
      // against molecules.css's live text during the t-206 rebase, not just
      // carried over from the original critic pass (which predates t-131's
      // round N and the .v2-avatar deletion above, so it needed re-checking
      // rather than trusting). The clamp mechanics below have no
      // molecules.css equivalent and stay live exactly as before.
