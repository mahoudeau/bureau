...
//
// t-133 (goal: t-53): +2 icons, both vendored the same way (fetched once,
// embedded literally, license recorded in icons/LICENSE — never loaded at
// runtime). "github": the ONE deliberate exception to "every glyph here is
// a Lucide/Feather outline" — a recognizable brand mark has to be a filled
// silhouette to read at 16-20px, so it carries fill="currentColor" instead
// of the set's usual stroke treatment; sourced from Simple Icons (CC0-1.0),
// the standard vendored source for monochrome brand marks, used ONLY for
// project rows whose repo host is literally github.com (see board.js's
// repoIconName()) — every other host renders "git-branch" below instead,
// so the UI never claims a brand it can't identify. "square-pen": a plain
// Lucide glyph (same ISC-derived source/style as the rest of this file),
// added for the project row's quiet inline-edit affordance on the repo
// field (project-edit.js's existing prompt()-free CRUD, t-86) now that the
// field's primary click opens the repo link instead of starting an edit.
